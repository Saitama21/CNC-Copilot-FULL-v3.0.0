# CNC Copilot FULL 3.0.3 — архитектура
...
Клиент объединяет локальные и серверные записи, но отдельно синхронизирует `syncMarks` для инструментов и проектов. Каждая метка содержит время изменения и состояние `deleted`. При конфликте используется более новая метка. Если последняя метка помечает запись удалённой, карточка отбрасывается даже если старая копия ещё присутствует в серверном пакете. Поэтому локальное удаление распространяется в Railway и не может быть автоматически «воскрешено» старым облачным состоянием. Повторное добавление или изменение создаёт новую активную метку. После успешной записи сервер увеличивает ревизию.
