...
- исправлена синхронизация удалений: инструмент, удалённый из «Умного шкафа», больше не возвращается из Railway при следующем merge;
- удаление проекта синхронизируется тем же способом;
- в пакет синхронизации добавлены локальные метки изменений `syncMarks` с временем и состоянием «удалено / активно»;
- повторное добавление или изменение позиции создаёт более новую активную метку, поэтому намеренно возвращённый инструмент не блокируется старым удалением;
- PWA-кэш обновлён до 3.0.3.

## Что изменено в 3.0.2

- новый расчёт и ручное добавление инструмента больше не проставляют «Торцовка» и «Наружка» автоматически;
- если пользователь не выбрал назначение, оно остаётся пустым;
- ИИ по-прежнему может предложить назначения только как результат распознавания.
