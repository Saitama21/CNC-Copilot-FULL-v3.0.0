...
function renderPreflight(){
  ...
  rows.push({ok:true,t:`Станок: ${m.name}, лимит расчёта ${max} об/мин, ${m.spindleKw} кВт`});
...
function sinumerikNote(res){const route=state.route.find(x=>x.uid===res.routeUid),op=operation(res.opId),max=effectiveMaxRpm();let extra='';
  ...
  return `${op.shopturn} ${extra} Лимит шпинделя для расчёта: ${max} об/мин.`;
}
...
window.CNC_APP={
  toast,
  getSyncPayload(){return {version:'3.0.6',machine:store.get(KEYS.machine,state.machine),tools:store.get(KEYS.tools,[]),projects:store.get(KEYS.projects,[]),draft:store.get(KEYS.draft,null),syncMarks:store.get(KEYS.syncMarks,{tools:{},projects:{}}),updatedAt:new Date().toISOString()}},
