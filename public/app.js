...
function initOfflineStatus(){const label=$('#offlineLabel');if(label)label.textContent='OFFLINE CORE · LOCAL'}
...
window.CNC_APP={
  toast,
  getSyncPayload(){return {version:'3.0.0',machine:store.get(KEYS.machine,state.machine),tools:store.get(KEYS.tools,[]),projects:store.get(KEYS.projects,[]),draft:store.get(KEYS.draft,null),updatedAt:new Date().toISOString()}},
